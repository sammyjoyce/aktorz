...
export class Runtime {
  ...
  /** Creates an in-memory runtime. Prefer Runtime.memory() for clarity. */
  constructor(options: RuntimeOptions | SQLiteRuntimeOptions = {}) {
    ...
    const nativeOptions: NativeRuntimeOptions = isSQLiteRuntimeOptions(options)
      ? {
          kind: "sqlite",
          snapshotEvery,
      ...
      : { kind: "memory", snapshotEvery }

    this.#bridge = new ServiceBridge()
    this.#native = new NativeRuntimeBinding(this.#bridge.dispatch, nativeOptions, options.libraryPath)
  }
...
function isSQLiteRuntimeOptions(options: RuntimeOptions | SQLiteRuntimeOptions): options is SQLiteRuntimeOptions {
  return Object.hasOwn(options, "path")
}
